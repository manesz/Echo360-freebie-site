...
/*  Copyright 2013  daddydesign.com  (email : daddydesign@gmail.com)
...
function DDST_new_tweets()
{
	...
	if (false === ( $fs_tweets = get_transient('DDST_socialtoolbar_1_5') )|| $social_icons[1]['url']!=get_transient('DDST_pro_tweets_user') ) {
	//if tweets are not in the cache
     $fs_tweets = DDST_get_tweets();//fetch them
     }
}
function DDST_get_tweets()
{
	require('oauth/StormTwitter.class.php');
        $social_icons=get_option('SOCIALTOOLBARICONS'); 
	...
	$config = array(
	  ...
	  'key' => 'xKwk5LkQu3MIuF1XhZYEfg',
	  'secret' => 'iKU8QcrNfFYVWYFsf47pbSUXIXgRNu0S40W2h6IwDQ',
	  'token' => '350670666-El8CSVzrwvVbGtzBU4Z5PWtfkRcBe6TcGHNmGYpb',
	  'token_secret' => 'Om4uzW36QV7xascXpGZMrAsYZ8BCjqmDvL9ZXs',
	  'screen_name' => $social_icons[1]['url'],
	  'tweet_count' => 1,
...
$tweets = $twitter->getTweets($config['tweet_count'],$config['screen_name'], 
array('include_rts'=>true,'exclude_replies'=>true,'trim_user'=>false,'include_entities'=>true));


function hyperlinks($text,$display_url) {
        $text = preg_replace('/\b([a-zA-Z]+:\/\/[\w_.\-]+\.[a-zA-Z]{2,6}[\/\w\-~.?=&%#+$*!]*)\b/i',"<a href=\"$1\" class=\"twitter-link\" target=\"_blank\">$display_url</a>", $text);
        $text = preg_replace('/\b(?<!:\/\/)(www\.[\w_.\-]+\.[a-zA-Z]{2,6}[\/\w\-~.?=&%#+$*!]*)\b/i',"<a href=\"http://$1\" class=\"twitter-link\">$display_url</a>", $text);
        // match name@address
        $text = preg_replace("/\b([a-zA-Z][a-zA-Z0-9\_\.\-]*[a-zA-Z]*\@[a-zA-Z][a-zA-Z0-9\_\.\-]*[a-zA-Z]{2,6})\b/i","<a href=\"mailto://$1\" class=\"twitter-link\" target=\"_blank\">$display_url</a>", $text);
            //mach #trendingtopics. Props to Michael Voigt
        $text = preg_replace('/([\.|\,|\:|\??|\??|\>|\{|\(]?)#{1}(\w*)([\.|\,|\:|\!|\?|\>|\}|\)]?)\s/i', "$1<a href=\"http://twitter.com/#search?q=$2\" class=\"twitter-link\" target=\"_blank\">#$2</a>$3 ", $text);
        return $text;
    }
...
for($i=0; $i < $config['tweet_count']; $i++)
{
   	if($tweets[$i]['text']){
		if (isset($tweets[$i]['retweeted_status'])){
			$msg = "RT  @".$tweets[$i]['retweeted_status']['user']['screen_name'].": ".$tweets[$i]['retweeted_status']['text'];
		}
		else{
		$msg = $tweets[$i]['text'];	 
		}
	}
	$sname=$tweets[$i]['user']['screen_name'];
	$link = "https://twitter.com/".$sname;
    $msg = hyperlinks($msg,$display_url);
	if (isset($tweets[$i]['retweeted_status'])){
		$msg = preg_replace('/@'.$tweets[$i]['retweeted_status']['user']['screen_name'].'/i', '<a href="http://www.twitter.com/'.$tweets[$i]['retweeted_status']['user']['screen_name'].'"target="_blank">@'.$tweets[$i]['retweeted_status']['user']['screen_name'].'</a>',
			$msg); 
	}
    $author=$tweets[$i]['user']['name'];
	
	if($author!=''){
		$results=$msg." via <a href='$link' target='_blank'>@".$sname."</a> ";
	}
}
if($results!=''){
